...
import 'package:abis2/api_handling/StockReportAPI.dart';
...
class StockReportsPage extends StatefulWidget {
  ...
  @override
  State<StockReportsPage> createState() => _StockReportsScreenState();
}

class _StockReportsScreenState extends State<StockReportsPage> {
  ...
  void initState() {
    super.initState();
    _stockData = StockReportAPI.fetchStock(loginBranchId, "VNBIN");
    startTimer();
    fetchBusinessDate();
  ...
  late Future<List<Map<String, dynamic>>> _stockData;

  int _sortColumnIndex = 0;
  bool _isAscending = true;

  void _sort<T>(Comparable<T> Function(Map<String, dynamic> stock) getField, int columnIndex, bool ascending) {
    _stockData = _stockData.then((list) {
      list.sort((a, b) {
                                     ...
                                     child: FutureBuilder<List<Map<String, dynamic>>>(
                                       ...
                                       builder: (context, snapshot) {
                                         if (snapshot.connectionState == ConnectionState.waiting) {
                                           ...
                                           return Center(child: Text("No stock available"));
                                         }
                                           ...
                                           columns: [
                                             DataColumn(
                                               ...
                                               onSort: (columnIndex, ascending) {
                                                 _sort<String>((stock) => stock["ItemID"], columnIndex, ascending);
                                               },
                                             ),
                                             DataColumn(
                                               label: Text('ItemName'),
                                               onSort: (columnIndex, ascending) {
                                                 _sort<String>((stock) => stock["ItemName"], columnIndex, ascending);
                                               },
                                             ),
                                             DataColumn(
                                               label: Text('BatchNumber'),
                                               onSort: (columnIndex, ascending) {
                                                 _sort<String>((stock) => stock["BatchNumber"] ?? "", columnIndex, ascending);
                                               },
                                             ),
                                             DataColumn(
                                               label: Text('CategoryName'),
                                               onSort: (columnIndex, ascending) {
                                                 _sort<String>((stock) => stock["CategoryName"], columnIndex, ascending);
                                               },
                                             ),
                                             DataColumn(
                                               label: Text('Bin Name'),
                                               onSort: (columnIndex, ascending) {
                                                 _sort<String>((stock) => stock["BinName"], columnIndex, ascending);
                                               },
                                             ),
                                             DataColumn(
                                               label: Text('Quantity'),
                                               numeric: true,
                                               onSort: (columnIndex, ascending) {
                                                 _sort<num>((stock) => stock["StockQty"], columnIndex, ascending);
                                               },
                                             ),
                                             DataColumn(
                                               label: Text('AltQty'),
                                               numeric: true,
                                               onSort: (columnIndex, ascending) {
                                                 _sort<num>((stock) => stock["StockAltQty"], columnIndex, ascending);
                                               },
                                             ),
                                           ],
                                           rows: stockList.map((stock) {
                                             return DataRow(cells: [
                                               DataCell(Text(stock["ItemID"])),
                                               DataCell(Text(stock["ItemName"])),
                                               DataCell(Text(stock["BatchNumber"] ?? "")),
                                               DataCell(Text(stock["CategoryName"])),
                                               DataCell(Text(stock["BinName"])),
                                               DataCell(Text(stock["StockQty"].toString())),
                                               DataCell(Text(stock["StockAltQty"].toString())),
                                             ]);
                                           }).toList(),
                                         );
                                       },
